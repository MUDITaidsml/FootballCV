...
def main():
    ...
    # Assign Ball Acquisition with safety checks
    player_assigner = PlayerBallAssigner()
    team_ball_control = []
    ball_frames = tracks.get('ball', [])
    for frame_num, player_track in enumerate(tracks.get('players', [])):
        # Retrieve ball bbox safely
        ball_frame = ball_frames[frame_num] if frame_num < len(ball_frames) else {}
        ball_bbox = ball_frame.get(1, {}).get('bbox')
        if ball_bbox is None:
            # No ball detection; repeat previous control or default to 1
